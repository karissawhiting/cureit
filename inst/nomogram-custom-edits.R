...
nomogram <- function(x,
    ...
    if (length(surv.factors) > 0 & length(surv.continuous) > 0){
      surv_nmmat <- data.frame(variables=c(unlist(mapply(rep,surv.factors,num.levels)),
                               ...
                               levels=c(unlist(x$surv_xlevels), 
                                        rep(NA, length(surv.continuous)*10)),
                               lp=0,
                               lpscale=0,
  ...
  if (cure){
    ...
    df_cov_cure <- data.frame(x=cure_nmmat$lpscale,
                              levels=cure_nmmat$levels,y=paste0("Cured probability: \n",
                                                                cure_nmmat$variables))
  ...
  all <- all %>%
    mutate(y = str_replace_all(y, "log_tumor_size", "Log Tumor Size (Size in cm)")) %>%
    mutate(levels_num = 2^(as.numeric(levels))) %>%
    mutate(levels_num = case_when(
      str_detect(y, "Tumor Size") ~ as.character(round(levels_num, 1)), 
      TRUE ~ levels)) %>%
    mutate(levels = case_when(
      str_detect(y, "Tumor Size") ~ glue::glue("{levels} ({levels_num})"), 
      TRUE ~ levels))
  ...
  p1 <- all %>%
    ...
    #  geom_text(aes(label = levels), vjust = 1.5, angle = angle)  + 
    ylab(" ") + xlab(" ") +
    
    # geom_text(aes(label = levels), vjust = 1.5, angle = angle) +
    ggrepel::geom_text_repel(aes(label = levels), size = 3,
                             #  max.overlaps = 3,
