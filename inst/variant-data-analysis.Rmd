...
object <- survfit2(Surv(os_months/12, dod) ~ variant_name_specify,
                 data = df) 

ggsurvfit(object) + 
  ggsurvfit::add_pvalue(location = c("annotation")) +
  add_risktable(risktable_stats = "n.risk") + 
  add_risktable_strata_symbol() + 
  add_confidence_interval() + 
  ylab("Disease-related Death \nProbability") + 
  xlab("Years") +
  xlim(c(0, 20)) +
  theme_classic() +
...
object %>% tbl_survfit(times = 5)
```

```{r}

object <- survfit2(Surv(os_months/12, dod) ~ 1,
                 data = df) 
